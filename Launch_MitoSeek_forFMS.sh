...
SAMPLE_PATH="/home/nhrg/WORKING_DATA/SourceBio/Reruns_2876/WTCCC"	  #directory containing all gzipped fastq files
LAUNCH_PATH="/home/nhrg/WORKING_DATA/SourceBio/Reruns_2876/WTCCC/jobs"  #dir from where this .exe is being run
...
for SampleFolder in "${INALL6[@]}"; do
        ...
        qsub -hold_jid "${JOB_ID}" -q "$BMQ" -N "${JOB_ID6}" -l "h_vmem=1G" -M "$EMAIL" ${SCRIPT_PATH}/MitoSeekv1-3_runPerl.sh "${arr6[@]}"
done
####

...
#when NO plus ref!! #if(($pos<=200 or $pos>=16000) and $chr=~/chrM/ and $pileup_file=~/mm10.pileup/){next ploop;} #use values from hg38plus for region covering origin of rCRS
...
####
## Add another file loop for hg38plus pileup to get coverage for bases 1-200 and 16000-16299
#my $pileupplus_file="NA";
#if($pileup_file=~/(\S+mm10).pileup/){
#	my $pre=$1;
...
#		if($ppos>200 and $ppos<16000){next pploop;}
#		$pos=$ppos;
#	#loop through chr specific targets
#	$Startfrom{$chr}=0;
#	for (my $count = $Startfrom{$chr}; $count<$T_perChr{$chr}; $count++){	

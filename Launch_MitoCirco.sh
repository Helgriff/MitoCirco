...
## Version 0.0.5 - Messy but functional analysis of large volumes of targeted mtDNA sequences + Exomes & Genomes supported with a few modifications ##
#***************************************************************************************************************************************************##

#***************************************************************************************************************************************************##
## Parameters that need to be adjusted for different batches of samples														
Batch_ID="SB_06April16"															   ## Sequence batch identifier
TYPE="Target"																   ## options are "Target", "Exome" or "Genome"
Fastqfile_prefix="NDCN"														##Common prefix at start of all fastq files
SAMPLE_PATH="/home/NDCN"	  #directory containing all gzipped fastq files
LAUNCH_PATH="/home/NDCN/jobs"  #dir from where this .exe is being run
SCRIPT_PATH="/home/Scripts/Pipeline_MitoCirco"	#directory containing all MitoCirco scripts
...
IntGENES="/home/Pipeline_MitoCirco/rCRS/hg38_MitochondrialGenes.txt"                  #File of candidate disease genes ##
...
ANV="/home/Pipeline_MitoCirco/annovar_jan2016"   #Pathway to Annovar perl scripts
...
for SampleFolder in "${INALL2[@]}"; do
	...
	qsub -N "${JOB_ID}" -pe smp "$THREADS" -l "$LMEM" -q "$BMQ" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoCirco1_AlignVars.sh "${arr[@]}"
	qsub -hold_jid "${JOB_ID}" -N "${JOB_ID6}" -q "$BMQ" -l "h_vmem=1G" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoSeekv1-3_runPerl.sh "${arr6[@]}"
	qsub -hold_jid "${JOB_ID6}" -N "${JOB_ID1b}" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoCirco1b_CircosPlots.sh "${arr1b[@]}"
        qsub -hold_jid "${JOB_ID},${JOBHOLD3}" -N "${JOB_ID2}" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoCirco2_MitoMaster.sh "${arr2[@]}"
...
qsub -hold_jid "${JOBHOLD}" -N "${JOB_ID3}" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoCirco3_Summary.sh "${arr3[@]}"
####
...
for SampleFolder in "${INALL2[@]}"; do
	...
	qsub -hold_jid "${JOB_ID3}" -N "${JOB_ID4}" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoCirco4_MoreVarCalls.sh "${arr4[@]}"
done
####
...
for SampleFolder in "${INALL5[@]}"; do
        ...
        qsub -hold_jid "${JOB_ID4}" -N "${JOB_ID5}" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/BashScripts/MitoCirco5_Annotation.sh "${arr5[@]}"
done
####

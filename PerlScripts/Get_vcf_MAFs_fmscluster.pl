...
my $path1="/home/nhrg/WORKING_DATA/SourceBio/BAMs"; ##path to directory containing vcf file
...
foreach my $p (@prefixes){
	...
	my $VCF_prefix=$p;
	my $VCF_suffix=$s;
	my $vcf_file=$path1."/".$VCF_prefix."/".$VCF_prefix.$VCF_suffix;
	my $numsamples=0;
	...
	foreach my $st (sort {$a<=>$b} keys %{$mafs{$ch}}){
		foreach my $r (sort keys %{$mafs{$ch}{$st}}){
			foreach my $v (sort keys %{$mafs{$ch}{$st}{$r}}){
				...
				print OUT2 "$ch\t$st\t$mafs{$ch}{$st}{$r}{$v}{'Call'}\t$r\t$v\t200\tPASS\tTS\=$totsamples\;TG\=$totgenos\;HmR\=$mafs{$ch}{$st}{$r}{$v}{'HomR'}\;Ht\=$mafs{$ch}{$st}{$r}{$v}{'Het'}\;HmV\=$mafs{$ch}{$st}{$r}{$v}{'HomV'}\;MAF\=$MAF\t\:A\:B\:C\:D\:E\:F\:G\:H\:I\:J\:K\:L\:M\:N\:P\:Q\:R\:T\:U\:V\:W\:X\:Y\:Z\:HV\:\t$mafs{$ch}{$st}{$r}{$v}{'HgpHomR'}\t$mafs{$ch}{$st}{$r}{$v}{'HgpHet'}\t$mafs{$ch}{$st}{$r}{$v}{'HgpHomV'}\n";
			}
		}
	}
}

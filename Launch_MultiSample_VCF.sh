...
SAMPLE_PATH="/home/nhrg/WORKING_DATA/SourceBio/BAMs/4771"
SCRIPT_PATH="/home/nhrg/WORKING_DATA/Pipeline_MitoCirco"
REF_BUILD="hg38"
REF_FILE="/users/nhrg/lustre/hg38/hg38.fa"
REF_BUILD2="hg38plus"
REF_FILE2="/users/nhrg/lustre/hg38_rCRS8330/hg38_rCRS8330.fa"
Q=25
MaxD=200
IntGENES="/users/nhrg/lustre/Pipeline_MitoCirco/rCRS/hg38_MitochondrialGenes.txt"
IHMAF="/home/nhrg/WORKING_DATA/InHouse_FastuniqBWAFreebayesPipe_Variants_MAFs_281files.txt.gz"
BMQ="all.q,bigmem.q"
LMEM="h_vmem=15G"
...
ANV="/home/nhrg/WORKING_DATA/Pipeline_MitoCirco/annovar_jan2016"
...
#qsub -N "${JOB_ID1b}" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/Samtools1-3Bcftools.sh "${arr14[@]}"
...
qsub -hold_jid "${JOBHOLD}" -N "${JOB_ID2b}" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/Bgzip_tabix.sh "${arr2b[@]}"
...
qsub -hold_jid "${JOBHOLD},${JOBHOLD2}" -N "${JOB_ID3}"  -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/Merge_vcfs.sh "${arr3[@]}"
qsub -hold_jid "${JOBHOLD},${JOBHOLD2}" -N "${JOB_ID3b}" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/Merge_vcfs.sh "${arr3b[@]}"
...
qsub -hold_jid "${JOBHOLD},${JOBHOLD2},${JOB_ID2},${JOB_ID2b},${JOB_ID3},${JOB_ID3b}" -N "${JOB_ID4}" -q "$BMQ" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/Combine_hg38_andplusVCFs.sh "${arr15[@]}"
...
qsub -hold_jid "${JOBHOLD},${JOBHOLD2},${JOB_ID2},${JOB_ID2b},${JOB_ID3},${JOB_ID3b},${JOB_ID4},${JOB_ID5}" -N "${JOB_ID6}" -q "$BMQ" -l "$LMEM" -M "$EMAIL" ${SCRIPT_PATH}/Get_genos_depths.sh "${arr6[@]}"
